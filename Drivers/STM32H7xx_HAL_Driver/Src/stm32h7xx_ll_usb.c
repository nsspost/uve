...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
volatile uint32_t usb_ll_iso_epstart_calls_dbg = 0U;
volatile uint32_t usb_ll_iso_ep_dbg = 0U;
volatile uint32_t usb_ll_iso_len_dbg = 0U;
volatile uint32_t usb_ll_iso_maxpacket_dbg = 0U;
volatile uint32_t usb_ll_iso_pktcnt_dbg = 0U;
volatile uint32_t usb_ll_iso_dsts_dbg = 0U;
volatile uint32_t usb_ll_iso_dieptsiz_after_program_dbg = 0U;
volatile uint32_t usb_ll_iso_diepctl_before_enable_dbg = 0U;
volatile uint32_t usb_ll_iso_diepctl_after_enable_dbg = 0U;
volatile uint32_t usb_ll_iso_diepctl_after_parity_dbg = 0U;
volatile uint32_t usb_ll_iso_dtxfsts_before_write_dbg = 0U;
volatile uint32_t usb_ll_iso_dtxfsts_after_write_dbg = 0U;
...
volatile uint32_t usb_ll_writepacket_count32b_dbg = 0U;
volatile uint32_t usb_ll_writepacket_first_word_dbg = 0U;
volatile uint32_t usb_ll_writepacket_dtxfsts_before_dbg = 0U;
volatile uint32_t usb_ll_writepacket_dtxfsts_after_dbg = 0U;
...
HAL_StatusTypeDef USB_EPStartXfer(USB_OTG_GlobalTypeDef *USBx, USB_OTG_EPTypeDef *ep, uint8_t dma)
{
  uint32_t USBx_BASE = (uint32_t)USBx;
  uint32_t epnum = (uint32_t)ep->num;
  uint32_t is_iso_in_ep1;
  uint16_t pktcnt;

  is_iso_in_ep1 = ((ep->is_in == 1U) && (epnum == 1U) && (ep->type == EP_TYPE_ISOC)) ? 1U : 0U;
  if (is_iso_in_ep1 != 0U)
  {
    usb_ll_iso_epstart_calls_dbg++;
    usb_ll_iso_ep_dbg = epnum;
    usb_ll_iso_len_dbg = ep->xfer_len;
    usb_ll_iso_maxpacket_dbg = ep->maxpacket;
    usb_ll_iso_dsts_dbg = USBx_DEVICE->DSTS;
  }
  ...
  if (ep->is_in == 1U)
  {
    ...
    if (is_iso_in_ep1 != 0U)
    {
      usb_ll_iso_dieptsiz_after_program_dbg = USBx_INEP(epnum)->DIEPTSIZ;
      usb_ll_iso_diepctl_before_enable_dbg = USBx_INEP(epnum)->DIEPCTL;
    }
                                  ...
                                  uint8_t ch_ep_num, uint16_t len, uint8_t dma)
{
  ...
  if (dma == 0U)
  {
    ...
    if (ch_ep_num == 1U)
    {
      usb_ll_writepacket_calls_dbg++;
      usb_ll_writepacket_ep_dbg = ch_ep_num;
      usb_ll_writepacket_len_dbg = len;
      usb_ll_writepacket_count32b_dbg = count32b;
      usb_ll_writepacket_first_word_dbg = (src != NULL) ? __UNALIGNED_UINT32_READ(src) : 0U;
      usb_ll_writepacket_dtxfsts_before_dbg = USBx_INEP((uint32_t)ch_ep_num)->DTXFSTS;
    }
    ...
    if (ch_ep_num == 1U)
    {
      usb_ll_writepacket_dtxfsts_after_dbg = USBx_INEP((uint32_t)ch_ep_num)->DTXFSTS;
    }
  }
