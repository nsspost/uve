...
void ILI9488_SendCommand(uint8_t com)
{
	...
	HAL_SPI_Transmit(&hspi5, &tmpCmd, 1, 1);
	//SendByte(tmpCmd);
	//WaitLastData();
	CS_D();
	//Bring CS HIGH
...
void ILI9488_Init_SPI_18bit(void)
{
    CS_D();

    // аппаратный reset, RESET активен низким уровнем
    RST_D();
    HAL_Delay(20);
    RST_A();
    HAL_Delay(20);
    ...
    lcd_cmd(0x3A);      // COLMOD
    lcd_data(0x66);     // 18-bit/pixel for SPI write test

    lcd_cmd(0x36);      // MADCTL
    lcd_data(0x48);     // BGR, без поворота

    lcd_cmd(0x29);      // DISPON
    HAL_Delay(50);
}

#define LCD_RGB_PROFILE  3   // 1, 2 или 3

void lcd_cmd(uint8_t v)
{
    DC_COMMAND();
    CS_A();
    HAL_SPI_Transmit(&hspi5, &v, 1, HAL_MAX_DELAY);
    CS_D();
}

void lcd_data(uint8_t v)
{
    DC_DATA();
    CS_A();
    HAL_SPI_Transmit(&hspi5, &v, 1, HAL_MAX_DELAY);
...
void lcd_data_buf(const uint8_t *buf, uint32_t len)
{
    DC_DATA();
    CS_A();
    HAL_SPI_Transmit(&hspi5, (uint8_t *)buf, len, HAL_MAX_DELAY);
    CS_D();
}


void lcd_reg(uint8_t reg, uint8_t data)
{
    lcd_cmd(reg);
    lcd_data(data);
}

void ILI9488_Init_RGB_From_WorkingSPI(void)
{
    CS_D();

    // HW reset: RESET active low
    RST_D();
    HAL_Delay(20);
    RST_A();
    HAL_Delay(20);
    ...
    // База, которая у тебя уже работает по SPI
    lcd_reg(0x3A, 0x55);      // COLMOD: 18-bit
    lcd_reg(0x36, 0x48);      // MADCTL: BGR, без поворота

    // --- Профили RGB ---
#if LCD_RGB_PROFILE == 1
    // Самый "осторожный"
    lcd_reg(0xB0, 0x00);      // Interface Mode Control
    lcd_cmd(0xB6);            // Display Function Control
    lcd_data(0x02);
    lcd_data(0x02);
    lcd_reg(0xE9, 0x00);      // disable 24-bit path
    lcd_reg(0xC6, 0x02);      // диагностический вариант RGB-related mode

#elif LCD_RGB_PROFILE == 2
    // Ближе к часто встречающимся init'ам для RGB-панелей
    lcd_reg(0xB0, 0x00);
    lcd_cmd(0xB6);
    lcd_data(0x32);
    lcd_data(0x02);
...
#elif LCD_RGB_PROFILE == 3
    // Ближе к твоему старому init, но без лишнего мусора
    lcd_reg(0xB0, 0x80);
    lcd_cmd(0xB6);
    lcd_data(0x73);
    lcd_data(0x02);
...
void ILI9488_FillRect_SPI_18bit(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                                uint8_t r6, uint8_t g6, uint8_t b6)
{
    if (w == 0 || h == 0) return;
    if ((uint32_t)x + w > 320) return;
    if ((uint32_t)y + h > 480) return;
    ...
    for (uint32_t i = 0; i < (uint32_t)w * h; i++) {
        HAL_SPI_Transmit(&hspi5, px, 3, HAL_MAX_DELAY);
    }
    CS_D();
}
...
void ILI9488_Init(void)
{
    ...
    // software reset
    ILI9488_SendCommand(0x01);   // SWRESET
    HAL_Delay(150);

    // sleep out
    ILI9488_SendCommand(0x11);   // SLPOUT
    HAL_Delay(150);

    // RGB565
    ILI9488_SendCommand(0x3A);   // COLMOD
    ...
    ILI9488_SendData(0x73);
    ILI9488_SendData(0x02);

    ILI9488_SendCommand(0xE9);   // Set Image Function
    ILI9488_SendData(0x00);

    ILI9488_SendCommand(0x21);   // inversion OFF (!!!!!!!)
    HAL_Delay(10);
    // display on
    ILI9488_SendCommand(0x29);   // DISPON
    HAL_Delay(50);
}
...
void fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
	...
	CS_A();
		while(cnt>0)
		{
			HAL_SPI_Transmit(&hspi5, frm_buf, buf_size, HAL_MAX_DELAY);
...
static inline void put_px(uint16_t x, uint16_t y, uint16_t color)
{
    if (x < LCD_W && y < LCD_H) {
        fb[y * LCD_W + x] = color;
    }
...
void draw_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
    ...
    for (uint16_t xx = 0; xx < w; xx++) {
        put_px(x + xx, y, color);
        put_px(x + xx, y + h - 1, color);
    }

    for (uint16_t yy = 0; yy < h; yy++) {
        put_px(x, y + yy, color);
        put_px(x + w - 1, y + yy, color);
    }
}
